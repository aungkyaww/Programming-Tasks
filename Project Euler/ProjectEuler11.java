...
//currentProduct = grid[i][j]*grid[i][j]*grid[i][j]*grid[i][j];
    public static void main(String[] args) {
      
        //20x20 grid of numbers
      int grid[][] = {{8, 2, 22, 97, 38, 15, 0, 40, 0, 75, 4, 5, 07, 78, 52, 12, 50, 77, 91, 8},
      ...
      //DIAG UP LEFT    grid[i][j], grid[i - 1][j - 1], grid[i - 2][j - 2], grid[i - 3][j - 3]
      //DIAG UP RIGHT   grid[i][j], grid[i - 1][j + 1], grid[i - 2][j + 2], grid[i - 3][j + 3]
      //DIAG DOWN RIGHT grid[i][j], grid[i + 1][j + 1], grid[i + 2][j + 2], grid[i + 3][j + 3]
      //DIAG DOWN LEFT  grid[i][j], grid[i + 1][j - 1], grid[i + 2][j - 2], grid[i + 3][j - 3]
    ...
    public static boolean checkUpLeft (int i, int j) {
        return checkUp(i,j) && checkLeft(i,j);
    }
    
    public static boolean checkUpRight (int i, int j) {
        return checkUp(i,j) && checkRight(i,j);
    }
    
    public static boolean checkDownLeft (int i, int j) {
        return checkDown(i,j) && checkLeft(i,j);
    }
    
    public static boolean checkDownRight (int i, int j) {
        return checkDown(i,j) && checkRight(i,j);
    }
